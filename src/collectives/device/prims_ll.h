...
class Primitives<T, RedOp, Fan, Direct, ProtoLL, P2p>:
  ...
  __device__ __forceinline__ void LLGenericOp(intptr_t srcIx, intptr_t dstIx, int nelem, bool postOp, float factor=-1, float peer_factor=-1) {
    ...
    while (nelem > 0) {
      ...
      if (SRC) {
        data = dl.loadFinish();
        if (SrcBuf == Input){
          peerData = applyPreOp(redOp, peerData, peer_factor);
          data = applyPreOp(redOp, data, factor);
        }
      }
      if (RECV) {
 ...
 public:
  ...
  __device__ void recvReduceCopySend(intptr_t inpIx, intptr_t outIx, int eltN, bool postOp=false, float factor=-1, float peer_factor=-1) {
    return LLGenericOp<1, 1, Input, Output>(inpIx, outIx, eltN, postOp, factor, peer_factor);
  }
};
