  ...
  __device__ __forceinline__ void runRing(ncclWorkElem *args) {
    ...
    for (ssize_t gridOffset = 0; gridOffset < size; gridOffset += loopSize) {
      ...
      for (int j=2; j<nranks-X; ++j) { //IMPL: nranks-X
        ...
        nelem = min(realChunkSize, size-offset);
        if (j>nranks-X-Y-1){
          prims.recvReduceCopySend(offset, offset, nelem, false, 1.0/2.0, 3.0/2.0); //THIS IS JUST A SPECIAL HARDCODED CASE FOR X=1 Y=1, N=4
        }
        else{
      ...
      offset = calcOffset(chunk);
      nelem = min(realChunkSize, size-offset);
      prims.recvReduceCopySend(offset, offset, nelem, false, 1.0/3.0, 2.0/3.0); // NO TIME TO MAKE IT GENERAL
