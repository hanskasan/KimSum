...
template<typename Fn, typename Pack>
__device__ __forceinline__ Pack applyPreOp(Fn fn, Pack a, float factor=-1) {
  return fromPack<Pack>(
    Apply_PreOp<Fn, sizeof(Pack)/sizeof(typename Fn::EltType)>
      ::preOp(fn, toPack(a), factor)
  );
}
...
struct Apply_PreOp {
  ...
  template<int Size>
  __device__ static BytePack<Size> preOp(Fn fn, BytePack<Size> a, float factor=-1) {
    ...
    if (!IsIdentity) {
      ...
      a.half[0] = Apply_PreOp<Fn, EltPerPack/2>::preOp(fn, a.half[0], factor);
      a.half[1] = Apply_PreOp<Fn, EltPerPack/2>::preOp(fn, a.half[1], factor);
    }
    return a;
  }
};
...
struct Apply_PreOp<FuncPreMulSum<T>, /*EltPerPack=*/1> {
  ...
  __device__ static BytePack<sizeof(T)> preOp(FuncPreMulSum<T> fn, BytePack<sizeof(T)> a, float factor=-1) {

    if (factor==-1){
      return toPack<T>(fromPack<T>(a) * fn.scalar);
    }
    else{
      return toPack<T>(fromPack<T>(a)*factor);
    }
...
struct Apply_PreOp<FuncPreMulSum<half>, /*EltPerPack=*/1> {
  static constexpr bool IsIdentity = false;
  __device__ static BytePack<sizeof(half)> preOp(FuncPreMulSum<half> fn, BytePack<sizeof(half)> a, float factor=-1) {
    #if __CUDA_ARCH__ >= 530 && __CUDA_ARCH__ != 610
  ...
  struct Apply_PreOp<FuncPreMulSum<half>, /*EltPerPack=*/2> {
    static constexpr bool IsIdentity = false;
    __device__ static BytePack<sizeof(half2)> preOp(FuncPreMulSum<half> fn, BytePack<sizeof(half2)> a, float factor=-1) {
...
// Apply_PreOp of FuncPreMulSum for bfloat16.
  ...
  struct Apply_PreOp<FuncPreMulSum<__nv_bfloat16>, /*EltPerPack=*/1> {
    ...
    __device__ static BytePack<sizeof(__nv_bfloat16)> preOp(
        FuncPreMulSum<__nv_bfloat16> fn, BytePack<sizeof(__nv_bfloat16)> a, float factor=-1
      ) {
      #if __CUDA_ARCH__ >= 800
    ...
    struct Apply_PreOp<FuncPreMulSum<__nv_bfloat16>, /*EltPerPack=*/2> {
      ...
      __device__ static BytePack<sizeof(__nv_bfloat162)> preOp(
          FuncPreMulSum<__nv_bfloat16> fn, BytePack<sizeof(__nv_bfloat162)> a, float factor=-1
        ) {
